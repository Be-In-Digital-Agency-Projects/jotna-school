...
export default function PublishedPage() {
  const exercises = useQuery(api.exercises.listAllPublished);
  const allTopicsMap = useTopicsMap();
  const unpublishExercise = useMutation(api.exercises.unpublish);
  ...
  const [unpublishingId, setUnpublishingId] = useState<Id<"exercises"> | null>(
    null,
  );
                ...
                <tr key={exercise._id} className="hover:bg-gray-50">
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                    {exercise.publishedAt
                      ? new Date(exercise.publishedAt).toLocaleDateString(
                          "fr-FR",
                        )
                      : "—"}
                  </td>
                  <td className="whitespace-nowrap px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        onClick={async () => {
                          setUnpublishingId(exercise._id);
                          setError(null);
                          try {
                            await unpublishExercise({ id: exercise._id });
                          } catch (err) {
                            setError(refusalMessage(err, "Erreur lors de la dépublication"));
                          } finally {
                            setUnpublishingId(null);
                          }
                        }}
                        disabled={unpublishingId === exercise._id}
                        className="rounded-lg p-2 text-gray-400 transition-colors hover:bg-amber-50 hover:text-amber-600 disabled:opacity-50"
                        title="Depublier"
                      >
                        {unpublishingId === exercise._id ? (
